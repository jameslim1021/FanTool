...
def get_team_totals():
    soup = make_soup("http://www.pro-football-reference.com/years/2016/")
...
for team in team_totals:
    ...
    if len(team_result) == 0:
        ...
        cur.execute("INSERT INTO TEAMS_TOTALS (TEAM_ID, SEASON, POINTS, TOTAL_YARDS, TURNOVERS, COMPLETIONS, ATTEMPTS, PASS_YARDS, PASS_TDS, INTERCEPTIONS, CARRIES, RUSH_YARDS, RUSH_TDS) \
              ...
              (team_id, 2016, int(team_totals[team]["points"]),int(team_totals[team]["total_yards"]),int(team_totals[team]["turnovers"]),int(team_totals[team]["completions"]),int(team_totals[team]["attempts"]),int(team_totals[team]["pass_yards"]),
