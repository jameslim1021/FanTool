...
for team in game_log_2014:
    url_info[team] = {}
    for game in game_log_2014[team]:
        url_info[team][game] = {}
        if len(game_log_2014[team][game]) > 0:
            url_info[team][game]["date"] = format_date(game_log_2014[team][game]["date"])
            if game_log_2014[team][game]["home"] == True:
                url_info[team][game]["city"] = url_team_map[team]
            else:
                url_info[team][game]["city"] = game_log_2014[team][game]["opponent"]
    ...
    for week in player_log[team]:
        for player in player_log[team][week]:
            ...
            if len(player_result) == 0:
                ...
                cur.execute("SELECT id from players_teams where player_id=%(player_id)s and team_id=%(team_id)s;",
                ...
                if len(players_teams_result) == 0:
                    cur.execute("INSERT INTO PLAYERS_TEAMS (PLAYER_ID, TEAM_ID, SEASON) \
                        ...
                        (player_id, team_id, 2014))

                    players_team_id = cur.fetchone()[0]
                else:
                    players_team_id = players_teams_result[0]

                    cur.execute("SELECT id from games where team_id=%(team_id)s and week=%(week)s;",
