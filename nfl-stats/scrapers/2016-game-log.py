...
for team in game_log_2016:
    for week in game_log_2016[team]:
        ...
        cur.execute("INSERT INTO GAMES (TEAM_ID, OPPONENT_ID, SEASON, WEEK, RESULT, RECORD, HOME, POINTS_FOR, POINTS_AGAINST, TOTAL_YARDS_FOR, PASS_YARDS_FOR, RUSH_YARDS_FOR, TURNOVERS_LOST, TOTAL_YARDS_AGAINST, PASS_YARDS_AGAINST, RUSH_YARDS_AGAINST, TURNOVERS_FORCED) \
            ...
            (team_id, opponent_id, 2016, week, game_log_2016[team][week]["result"],game_log_2016[team][week]["record"],game_log_2016[team][week]["home"],
            int(game_log_2016[team][week]["points_for"]), int(game_log_2016[team][week]["points_against"]),int(game_log_2016[team][week]["total_yards_for"]),int(game_log_2016[team][week]["pass_yards_for"]),
            int(game_log_2016[team][week]["rush_yards_for"]),int(game_log_2016[team][week]["turnovers_lost"]),int(game_log_2016[team][week]["total_yards_against"]),
            int(game_log_2016[team][week]["pass_yards_against"]), int(game_log_2016[team][week]["rush_yards_against"]), int(game_log_2016[team][week]["turnovers_forced"])))
